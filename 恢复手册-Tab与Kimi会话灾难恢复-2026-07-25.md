...
桌面有现成的双击脚本 `~/Desktop/恢复kimi会话.command`（教程见 [一键恢复脚本-双击即用教程.md](一键恢复脚本-双击即用教程.md)）：自动拉起 Otty、等待 tab 就绪、跳过忙碌 pane、按目录分配不同会话并注入。**但它只能恢复"还存在"的空闲 tab**；若 tab 整个丢失（坑 2 场景），需按本文第 2 步先补 tab 再恢复。
...
- [重启后恢复聊天记录-kimi-c技巧.md](重启后恢复聊天记录-kimi-c技巧.md)
- [一键批量恢复终端Tab的Kimi会话-原理篇.md](一键批量恢复终端Tab的Kimi会话-原理篇.md)
- [一键恢复脚本-双击即用教程.md](一键恢复脚本-双击即用教程.md)
